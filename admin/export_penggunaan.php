<?php 
	...
	header("Content-disposition: attachment; filename=laporanpenggunaanlistrik.xls");
 ?>
		...
		<table cellspacing="1" width="90%" align="center" border="1">
			...
			<tbody>
				...
				if (($_GET['id_bulan'] == "") AND ($_GET['id_tahun'] == "")) {
					...
					$sql = mysql_query("SELECT * FROM query_penggunaan WHERE bulan = '$_GET[id_bulan]' ORDER BY id_penggunaan ASC");
				} elseif ($_GET['id_bulan'] == "") {
					$sql = mysql_query("SELECT * FROM query_penggunaan WHERE tahun = '$_GET[id_tahun]' ORDER BY id_penggunaan ASC");
				} else {
					$sql = mysql_query("SELECT * FROM query_penggunaan WHERE bulan = '$_GET[id_bulan]' AND tahun = '$_GET[id_tahun]' ORDER BY id_penggunaan ASC");
				}
